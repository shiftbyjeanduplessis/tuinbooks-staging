...
(()=>{
'use strict';
const BUILD='60.7.6-v4-eight-sheet-import-contract';
...
function v4HeaderObjects(rows,required){
 const wanted=(required||[]).map(key),list=rows||[];
 const idx=list.findIndex(row=>Array.isArray(row)&&wanted.every(name=>row.some(cell=>key(cell)===name)));
 if(idx<0)return[];
 const headers=(list[idx]||[]).map(clean);
 return list.slice(idx+1).map((row,i)=>{const out={_row:Number(row?._row)||idx+i+2};headers.forEach((header,column)=>{if(header)out[header]=row?.[column]??'';});return out;});
}
function legacyRows(headers,objects){return [[],[],[],headers,...objects.map(row=>headers.map(header=>row[header]??''))];}
function v4ScheduleWeeks(location){
 const f=key(location.Frequency),cycle=key(location['Fortnightly Cycle']),pattern=clean(location['Monthly Pattern']);
 if(f.includes('fortnight'))return cycle.includes('b')||cycle.includes('2')?[2,4]:[1,3];
 if(f.includes('month')){const match=pattern.match(/[1-4]/);return [match?Number(match[0]):1];}
 if(f.includes('week'))return [1,2,3,4];
 return [];
}
function adaptV4Workbook(parsed){
 if(!parsed?.sheets?.['1 Business Info'])return parsed;
 const info=startMap(rowsFor(parsed,'1 Business Info'));
 const services=v4HeaderObjects(rowsFor(parsed,'2 Services'),['Service Name']).filter(row=>clean(row['Service Name']));
 const teams=v4HeaderObjects(rowsFor(parsed,'3 Teams'),['Team Name']).filter(row=>clean(row['Team Name']));
 const accounts=v4HeaderObjects(rowsFor(parsed,'4 Clients Accounts'),['Client / Account Name']).filter(row=>clean(row['Client / Account Name']));
 const locations=v4HeaderObjects(rowsFor(parsed,'5 Service Locations'),['Client / Account','Location Name','Address']).filter(row=>clean(row['Client / Account'])||clean(row['Location Name'])||clean(row.Address));
 const routeRows=v4HeaderObjects(rowsFor(parsed,'8 Route Order'),['Team','Day','Week','Client / Account','Location Name']).filter(row=>clean(row['Client / Account'])||clean(row['Location Name']));
 const accountByName=new Map(accounts.map(row=>[key(row['Client / Account Name']),row]));
 const locationsByAccount=new Map();
 locations.forEach(row=>{const k=key(row['Client / Account']);if(!locationsByAccount.has(k))locationsByAccount.set(k,[]);locationsByAccount.get(k).push(row);});
 const routeOrder=new Map(routeRows.map(row=>[
   [key(row['Client / Account']),key(row['Location Name']),key(row.Week),key(row.Day)].join('|'),
   clean(row['Approved Stop']||row['Suggested Stop'])
 ]));
 const clientRows=accounts.map(account=>{
   const name=clean(account['Client / Account Name']),sites=locationsByAccount.get(key(name))||[],primary=sites[0]||{};
   return {
     'Client Name':name,'Contact Name':account['Contact Person'],'Phone':account.Phone,'Email':account.Email,
     'Address':primary.Address,'Suburb':primary['Suburb / Area'],'Status':'Active','Visit Frequency':primary.Frequency||'Custom',
     'Preferred Day':primary['Service Day 1'],'Team':primary.Team,'Notes':account['Billing Notes'],'TuinBooks ID':account['TuinBooks ID']||''
   };
 });
 const clientServices=locations.map(location=>({'Client':location['Client / Account'],'Service':location.Service,'Notes':location['Access / Service Notes'],'Location Name':location['Location Name']}));
 const schedule=[];
 locations.forEach(location=>{
   const days=['Service Day 1','Service Day 2','Service Day 3'].map(name=>clean(location[name])).filter(Boolean);
   const weeks=v4ScheduleWeeks(location);
   weeks.forEach(week=>days.forEach(day=>schedule.push({
     'Client':location['Client / Account'],'Team':location.Team,'Week':week,'Day':day,
     'Notes':location['Routing / Timing Notes'],'Location Name':location['Location Name'],
     'Route Order':routeOrder.get([key(location['Client / Account']),key(location['Location Name']),key(`Week ${week}`),key(day)].join('|'))||'',
     'Start Date':location['Start Date']||''
   })));
 });
 const billing=accounts.map(account=>({
   'Client':account['Client / Account Name'],'Billing Profile':'','Routine Charge':account['Account Billing Amount'],
   'Charge Basis':account['Billing Basis'],'Notes':account['Billing Notes'],'Invoice Day (optional)':account['Invoice Day']
 }));
 const rawMode=key(sv(info,'TuinBooks Mode')||sv(info,'Mode'));
 const mode=rawMode.includes('planning')&&!rawMode.includes('financial')?'PLANNING ONLY':'FULL SERVICE';
 const sheets={...parsed.sheets};
 sheets.Setup=[['Business Name',sv(info,'Business Name')],['Main Contact',sv(info,'Contact Person')],['Phone',sv(info,'Phone')],['Email',sv(info,'Email')],['Physical Address',sv(info,'Business Address')],['Town / Suburb',sv(info,'Suburb / Town')],['Province',sv(info,'Province')],['Country','South Africa'],['Currency','ZAR'],['Mode',mode]];
 sheets.Services=legacyRows(['Service','Notes'],services.map(row=>({Service:row['Service Name'],Notes:row.Notes})));
 sheets.Teams=legacyRows(['Team Name','Notes'],teams);
 sheets.Clients=legacyRows(['Client Name','Contact Name','Phone','Email','Address','Suburb','Status','Visit Frequency','Preferred Day','Team','Notes','TuinBooks ID'],clientRows);
 sheets['Client Services']=legacyRows(['Client','Service','Notes','Location Name'],clientServices);
 sheets.Schedule=legacyRows(['Client','Team','Week','Day','Notes','Location Name','Route Order','Start Date'],schedule);
 sheets.Check=[[],[],[],['Check'],['V4 contract adapted']];
 sheets['Client Billing']=legacyRows(['Client','Billing Profile','Routine Charge','Charge Basis','Notes','Invoice Day (optional)'],billing);
 return {...parsed,sheets,__v4:{accounts,locations,accountByName}};
}
...
function model(parsed,week1){
 ...
 if(financial){
   ...
   const billed=new Set();billing.forEach(r=>{const c=resolveClient(r),p=resolveProfile(r),charge=money(get(r,'Routine Charge')),rawBasis=clean(get(r,'Charge Basis')),basisKey=key(rawBasis),day=clean(get(r,'Invoice Day (optional)'));if(!c)blockers.push(`Client Billing row ${r._row}: client “${clean(get(r,'Client'))}” was not found.`);if(!p&&!(parsed.__v4&&!clean(get(r,'Billing Profile'))))attention(`billing-profile-missing:${key(get(r,'Client'))}`,'client-billing',`${clean(get(r,'Client'))} — billing profile`,`Billing profile “${clean(get(r,'Billing Profile'))}” was not found.`,['Open Import / Export.','Export the current workbook.','Choose an existing Billing Profile for this client in Client Billing.','Re-import.'],{clientName:clean(get(r,'Client')),action:'import'});const problems=[];if(!Number.isFinite(charge)||charge<0)problems.push('routine charge');if(!(basisKey==='per visit'||basisKey.startsWith('monthly fixed')))problems.push('charge basis');if(problems.length&&c)attention(`client-billing:${key(get(c,'Client Name'))}`,'client-billing',`${clean(get(c,'Client Name'))} — routine billing`,`${problems.join(' and ')} ${problems.length===1?'needs':'need'} confirmation.`,['Open Import / Export.','Export the current business workbook.','On Client Billing, choose Per Visit or Monthly Fixed and enter the agreed Routine Charge.','Re-import the corrected workbook.'],{clientName:clean(get(c,'Client Name')),action:'import'});if(day&&!parsed.__v4)warnings.push(`Client Billing row ${r._row}: Invoice Day is ignored. TuinBooks uses the business-wide invoice cycle.`);if(c)billed.add(clientIdentity(c));});if(parsed.__v4&&billing.some(r=>clean(get(r,'Invoice Day (optional)'))))warnings.push('Account Invoice Day values are informational. TuinBooks uses the business-wide invoice cycle.');clients.forEach(r=>{if(normStatus(get(r,'Status'))==='active'&&!billed.has(clientIdentity(r)))attention(`client-billing:${key(get(r,'Client Name'))}`,'client-billing',`${clean(get(r,'Client Name'))} — routine billing`,`No Client Billing row is set up.`,['Open Import / Export.','Export the current business workbook.','Add this client to Client Billing with the correct profile, basis and amount.','Re-import the corrected workbook.'],{clientName:clean(get(r,'Client Name')),action:'import'});});
 }
 ...
 return {parsed,setup,mode,financial,businessName,services,teams,clients,clientServices,schedule,billProfiles,billing,v4:parsed.__v4||null,week1,issues:[...new Set(blockers)],blockingIssues:[...new Set(blockers)],attention:attentionItems,warnings:[...new Set(warnings)],resolveClient,resolveService,resolveTeam,resolveProfile,clientIdentity};
}
...
function importPayloadV6074(s,importedJobs,scopeClientIds,{replaceClients,importStart,importEnd,expectedTeams,expectedCustomers,expectedSites,expectedSchedules}={}){
 ...
 const sites=(s.clients||[]).flatMap(client=>{
   const rows=Array.isArray(client.serviceSitesV56)&&client.serviceSitesV56.length?client.serviceSitesV56:[{id:client.siteId||`site-${client.id}`,name:client.siteName||client.name,address:client.address,suburb:client.suburb,clusterId:client.clusterId,accessNotes:client.accessNotes,petNotes:client.petNotes||client.gardenNotes,instructions:client.serviceDescription,active:client.status!=='archived'}];
   return rows.map(site=>({
     id:String(site.id||''),customer_id:String(client.id||''),site_name:clean(site.name||site.siteName||client.name),address:clean(site.address),suburb:clean(site.suburb),cluster_id:validClusterIds.has(String(site.clusterId||''))?String(site.clusterId):null,access_notes:clean(site.accessNotes),pet_notes:clean(site.petNotes||site.gardenNotes),instructions:clean(site.instructions||site.serviceDescription),active:site.active!==false&&client.status!=='archived',payload:{...site,id:String(site.id||''),customerId:String(client.id||''),siteName:clean(site.name||site.siteName||client.name),active:site.active!==false&&client.status!=='archived'}
   }));
 ...
 const payload={
   ...
   p_expected_site_count:Number(expectedSites||expectedCustomers||0),p_expected_schedule_count:Number(expectedSchedules||0),
...
async function apply(model,options={}){
 ...
 try{
   ...
   const siteByLocationKey=new Map();
   if(model.v4){
     const grouped=new Map();
     model.v4.locations.forEach(location=>{
       const clientId=clientByName.get(key(location['Client / Account']));if(!clientId)return;
       if(!grouped.has(clientId))grouped.set(clientId,[]);
       grouped.get(clientId).push(location);
     });
     grouped.forEach((locations,clientId)=>{
       const client=s.clients.find(row=>String(row.id)===String(clientId));if(!client)return;
       const existing=Array.isArray(client.serviceSitesV56)?client.serviceSitesV56:[];
       const sites=locations.map((location,index)=>{
         const name=clean(location['Location Name'])||`Service location ${index+1}`,address=clean(location.Address),suburb=clean(location['Suburb / Area']);
         let site=existing.find(row=>key(row.name||row.siteName)===key(name)&&key(row.address)===key(address));
         if(!site)site={id:uid('site'),createdAt:new Date().toISOString()};
         Object.assign(site,{name,address,suburb,accessNotes:clean(location['Access / Service Notes']),instructions:clean(location['Routing / Timing Notes']),primary:index===0,active:true,updatedAt:new Date().toISOString()});
         siteByLocationKey.set(`${key(location['Client / Account'])}|${key(name)}`,site.id);
         return site;
       });
       client.serviceSitesV56=sites;
       const primary=sites[0];if(primary){client.siteId=primary.id;client.siteName=primary.name;client.address=primary.address;client.suburb=primary.suburb;client.accessNotes=primary.accessNotes;}
     });
   }
   ...
   model.schedule.forEach(r=>{const cid=resolveClientId(r),tid=resolveTeamId(r),week=Number(get(r,'Week')),day=clean(get(r,'Day')),client=s.clients.find(c=>c.id===cid);if(!cid||!tid||client?.status!=='active'||![1,2,3,4].includes(week)||!DAYS.includes(day))return;const date=addDays(model.week1,(week-1)*7+DAYS.indexOf(day)),jobHidden=clean(get(r,'TuinBooks Schedule ID'))||clean(get(r,'TuinBooks ID')),slotKey=`${week}|${day}|${tid}`,fallbackSort=(routeSort.get(slotKey)||0)+1,sort=Number(get(r,'Route Order'))||fallbackSort;routeSort.set(slotKey,Math.max(fallbackSort,sort));const locationName=clean(get(r,'Location Name')),serviceSiteId=siteByLocationKey.get(`${key(get(r,'Client'))}|${key(locationName)}`)||client.siteId||'';let job=jobHidden?s.schedules.find(x=>String(x.id)===jobHidden):s.schedules.find(x=>x.clientId===cid&&x.date===date&&String(x.serviceSiteId||x.siteId||'')===String(serviceSiteId)&&(x.workKind==='recurring'||x.workMarker==='R')&&!['cancelled','canceled'].includes(key(x.status)));if(!job){job={id:uid('sch'),createdAt:new Date().toISOString()};s.schedules.push(job);}Object.assign(job,{date,clientId:cid,teamId:tid,serviceSiteId,siteId:serviceSiteId,siteName:locationName,status:'scheduled',estimatedHours:Number(client?.estimatedHours||1),sort,workKind:'recurring',workMarker:'R',revenueType:'Recurring contract',serviceIds:[...(client?.serviceIds||[])],officeNotes:clean(get(r,'Notes')),startTimeV60426:clean(get(r,'Start Time')),autoGenerated:false,manualOverride:true,onboardingMasterV60426:true,updatedAt:new Date().toISOString()});importedJobs.push(job);if(!clientFirst.has(cid)||date<clientFirst.get(cid))clientFirst.set(cid,date);});
   ...
   const payload=importPayloadV6074(s,importedJobs,scopeClientIds,{replaceClients,importStart,importEnd,expectedTeams:model.teams.length,expectedCustomers:model.clients.length,expectedSites:model.v4?.locations?.length||model.clients.length,expectedSchedules:model.schedule.length});
